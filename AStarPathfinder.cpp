...
AStarPathFinder::AStarPathFinder(){
	pathBank[0] = new int[500];
	pathBank[1] = new int[500];
}

AStarPathFinder::~AStarPathFinder(){
}

int AStarPathFinder::FindPath(int pathfinderID, int startingX, int startingY, int targetX, int targetY)
{
	...
	if (startX == targetX && startY == targetY && pathLocation[pathfinderID] > 0) return found;
	if (startX == targetX && startY == targetY && pathLocation[pathfinderID] == 0) return nonexistent;

	//	If target square is unwalkable, return that it's a nonexistent path.
	if (walkability[targetX][targetY]) goto noPath;
						...
						if (whichList[a][b] != onClosedList) {
							...
							if (!walkability[a][b]) {
								...
								if (a == parentXval - 1)
								{
									if (b == parentYval - 1)
									{
										if (!walkability[parentXval - 1][parentYval] || !walkability[parentXval][parentYval - 1]) corner = false;
									}
									else if (b == parentYval + 1)
									{
										if (!walkability[parentXval][parentYval + 1] || !walkability[parentXval - 1][parentYval]) corner = false;
									}
								}
...
void AStarPathFinder::ReadPath(int pathfinderID, int currentX, int currentY, int pixelsPerFrame)
{
	/*
...
int AStarPathFinder::ReadPathX(int pathfinderID, int pathLocation)
{
	int x;
...
int AStarPathFinder::ReadPathY(int pathfinderID, int pathLocation)
{
	int y;
	if (pathLocation <= pathLength[pathfinderID])
	{
